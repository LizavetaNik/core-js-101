...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return rect1.left < right2 && right1 > rect2.left && rect1.top < bot2 && bot1 > rect2.top;
}
...
function findFirstSingleChar(str) {
  const charCount = str.split('').reduce((count, char) => {
    ...
    updatedCount[char] = (updatedCount[char] || 0) + 1;
    return updatedCount;
  }, {});
...
function evaluateTicTacToePosition(position) {
  ...
  if (winningCombinations.some((combination) => combination.every((cell) => cell === 'X'))) {
    return 'X';
  }

  if (winningCombinations.some((combination) => combination.every((cell) => cell === '0'))) {
    return '0';
  }
